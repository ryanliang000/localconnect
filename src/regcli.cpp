...
char *get_apply_buffer() {
  ...
  if (buffer[0] == 0) {
    ...
    char len =
        strlen(reguser) < c_username_max ? strlen(reguser) : c_username_max;
...
int cb_proc_accept(int srvfd, int filter) {
  ...
  if (memcmp(buffer, reply, sizeof reply) != 0) {
    buffer[n] = 0;
    LOG_E("recv msg from regsrv[%d]: %s-%s, connect bind failed!", n, buffer,
          buffer2hex(buffer, n > 32 ? 32 : n));
    cb_proc_error(srvfd, filter);
    return -1;
  }
...
void regclient() {
  ...
  while (true) {
    ...
    if (send(regfd, applybuffer, c_oper_size, 0) != c_oper_size) {
      LOG_E("send to remote error[%d-%s] occur, ignored!", errno,
            strerror(errno));
      close(regfd);
      continue;
    }
...
#else
std::thread threads[c_regcli_max + 1];
void try_regclient() {
  while (regcli_wait < c_regcli_max_wait && regcli_curr < c_regcli_max) {
    threads[regcli_curr] = std::thread(regclient);
    threads[regcli_curr].detach();
    regcli_curr++;
    regcli_wait++;
  }
  LOG_R("regclient: curr-%d, wait-%d", regcli_curr, regcli_wait);
...
#ifndef REGFIX
  if (argc < 6) {
    err_quit("Usage: %s regserver regserverport username encodekey forwardip "
             "forwardport\n",
             argv[0]);
  }
