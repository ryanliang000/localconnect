...
struct pairfd {
  ...
  void setused(int f) {
    ...
    if (fd != -1) {
      fdmap[fd] = dest;
      tsocks[fd] = tsock(fd, dest, 2);
    }
    if (dest != -1) {
      fdmap[dest] = fd;
      tsocks[dest] = tsock(dest, fd, 1);
    }
  }
  void setunused() {
    if (fd != -1) {
...
int apply_client_fd(int fd, const char *buffer) {
  ...
  LOG_R("apply serv client fail, no unused serv client for exist for %s<%d>",
        buffer + 1, fd);
  return -1;
}
...
int cb_proc_accept(int srvfd, int filter) {
  ...
  if ((clifd = accept(srvfd, (sockaddr *)&cli, &clilen)) < 0) {
    LOG_E("accept[clifd-%d-clilen-%d] error[%d-%s], ignored!", clifd, clilen,
...
int cb_proc_recv(int fd, int filter) {
  ...
  if (fdmap[fd] == -1) {
    ...
    if ((n = recv(fd, buffer, c_oper_size, 0)) != c_oper_size) {
      LOG_E("proc first message return failed, length[%d], excpected[%d]", n,
            c_oper_size);
    ...
    if (memcmp(buffer, c_keyword_apply, c_keyword_len) == 0) {
      ...
      LOG_E("recv msg from client(%d): %s-0x%s, keyword not right, close "
            "connect!",
            n, buffer, buffer2hex(buffer, n > 32 ? 32 : n));
      proc_close_s(fd, filter);
    }
  } else {
